...
Two outlined buttons under **End day** — **Arrive hospital** and **Leave
hospital** — record a timestamp and nothing else. They are deliberately styled
apart from the task tiles and from End day, because they neither start nor stop
anything. A toast confirms the time, since nothing else on screen reacts.

The same thing is available as a link, for a Shortcuts automation or an NFC tag:
...
# frontend in a real browser against a mock backend (55 checks).
